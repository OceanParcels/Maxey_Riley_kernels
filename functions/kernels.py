...
def MRAdvectionRK4_2D(particle, fieldset, time):
    ...
    history term, Faxen corrections and sinking or floating force.
    The equation is numerically integrated using the 4th order runge kutta
    scheme for a 2nd order ODE equation.

    We appromate the time derivative at t (1rst step rk4) with a forward finite
    difference and the time derivative at t+delta_t (4th step rk4) with a
    backward finite difference.

    dependencies:
    - particle: 
        - up, vp: velocity of particle at time t
        - tau: stokes relaxation time (constant in time)
        - B: buoyancy particle (constant in time)

    - fieldset: 
        - Omega_earth: angular velocity earth (in rad/s, 0 for flat simulation)
        - delta_x, delta_y: stepsize for finite difference method
          gradients 
    """
...
def MRSMAdvectionRK4_2D(particle, fieldset, time):
    ...
    We appromate the time derivative at t (1rst step rk4) with a forward
    finite difference and the time derivative at t+delta_t (4th step rk4)
    with a backward finite difference.

    dependencies:
    - particle:
        - tau: stokes relaxation time
        - B: byoyancy of the particle
    - fieldset:
        - Omega_earth: angular velocity earth (in rad/s, 0 for flat simulation)
        - delta_x, delta_y: stepsize for finite difference method
          gradients 
...
def MRAdvectionRK4_3D(particle, fieldset, time):
    ...
    history and Faxen corrections. The equation is numerically integrated
    using the 4th order runge kutta scheme for a nonoverdamped equation.

    We appromate the derivative at t (1st step RK4) with a forward time
    derivative and the derivative at t+delta_t (4th step rk4) with a backward
    time derivative.

    dependencies:
    - particle: 
        - up, vp, wp: velocity components particle at time t
        - tau: stokes relaxation time (constant in time)
        - B: buoyancy particle (constant in time)
    - fieldset:
        - Omega_earth: angular velocity earth (in rad/s, 0 for flat simulation)
        - delta_x, delta_y, delta_z: stepsize for finite difference method
          gradients 
...
def MRSMAdvectionRK4_3D(particle, fieldset, time):
    ...
    and Faxen corrections. The equation is numerically integrated
    using the 4th order runge kutta scheme for an overdamped equation.
    (ODE order 1). This version adds w0 (g*tau) as first order term.
    ...
    - fieldset:
        ...
        - delta_x, delta_y, delta_z: stepsize for finite difference method
          gradients 
    """
    # Bterm_tau = (3. / (1. + 2. * particle.B)) * particle.tau
    Bterm_tau = 2 * (1 - particle.B) / (1. + 2 * particle.B) * particle.tau
    # w0 = (2 * (1 - particle.B) / (1. + 2 * particle.B)
    #       * fieldset.g * particle.tau)
    ...
    w1 = wf1 + Bterm_tau * (DwDt1 + fieldset.g) #- w0tau * dwdz1
    ...
    w2 = wf2 + Bterm_tau * (DwDt2 + fieldset.g) #- w0tau * dwdz2
    ...
    w3 = wf3 + Bterm_tau * (DwDt3 + fieldset.g) #- w0tau * dwdz3
   ...
   # calculate time derivative of fluid field
    ...
    w4 = wf4 + Bterm_tau * (DwDt4 + fieldset.g) #- w0tau * dwdz4
...
def MRSMAdvectionRK4_3D_v2(particle, fieldset, time):
    """
    Advection of particles using the slow manifold approximation
    of the Maxey-Riley equation in 3D without Basset history term
    and Faxen corrections. The equation is numerically integrated
    using the 4th order runge kutta scheme for an overdamped equation.
    (ODE order 1) This version treats w0 as zeroth order, adding the
    term tau*w0*du/dz in first order.
